...
template <int Arch, int kHeadDim, int kHeadDimV, int ClusterM, typename Element, typename ElementOut,
          ...
          bool PackGQA, bool Split, bool V_colmajor, bool Is_arbitrary, int kNFunc>
void run_flash_fwd(Flash_fwd_params &params, cudaStream_t stream) {
    ...
    static constexpr std::tuple<int, int, bool, bool> kBlockMN_RS_IntraWGOverlap = tile_size_fwd_sm90(kHeadDim, kHeadDimV, Is_causal, Is_local, Is_arbitrary, sizeof(Element) /*element_size*/, V_colmajor, PagedKVNonTMA, Has_softcap);
    static constexpr std::tuple<int, int, int, int, bool> kBlockMN_kNWarps_Stages_RS = tile_size_fwd_sm8x(Arch == 86 || Arch == 89, kHeadDim, kHeadDimV, Is_causal, Is_local, Is_arbitrary, sizeof(Element) /*element_size*/, PagedKVNonTMA, Varlen && Split, Has_softcap, AppendKV);
    ...
    static constexpr int kStages = Arch >= 90 ? 2
        : (Is_arbitrary && Arch == 80 && kHeadDim > 128 && kHeadDim <= 192
            ? 1
    ...
    using CollectiveMainloop = std::conditional_t<
        Arch >= 90,
        flash::CollectiveMainloopFwdSm90<kStages, ClusterShape, TileShape_MNK, kHeadDimV, Element, float, cutlass::arch::Sm90, Is_causal, Is_local, Has_softcap, Varlen, PagedKVNonTMA, AppendKV, HasQv, MmaPV_is_RS, IntraWGOverlap, PackGQA, Split, V_colmajor, Is_arbitrary, kNFunc>,
        flash::CollectiveMainloopFwdSm80<kNWarps, kStages, Q_in_regs, TileShape_MNK, kHeadDimV, Element, float, cutlass::arch::Sm80, Is_causal, Is_local, Has_softcap, Varlen, PagedKVNonTMA, AppendKV, PackGQA, Split, Is_arbitrary, kNFunc>
    >;
    ...
    using SchedulerPersistent = std::conditional_t<Varlen,
        ...
        std::conditional_t<!Is_causal && !Is_local && !Is_arbitrary,
    ...
    static constexpr bool UsePersistentScheduler = Arch >= 90 ? !(Split && !Varlen) : (((Is_causal || Is_arbitrary) && !Varlen) || (Varlen && Split));
    ...
    typename CollectiveMainloop::Arguments mainloop_args {
        ...
        {params.func_seqlen, params.arbitrary_func_num, params.func_head, params.func_batch},  // shape_mask_func: (seqlen_q + 256, func_num, head_q or 1, batch or 1)
        {_1{}, params.func_nfunc_stride, params.func_head_stride, params.func_batch_stride}  // stride_mask_func
    };
    // Block sparsity arguments (similar to scheduler_args)
                    ...
                    BOOL_SWITCH(params.qv_ptr, HasQV_, [&] {
                        ...
                        APPENDKV_SWITCH(params.knew_ptr, AppendKV, [&] {
                            ...
                            CLUSTER_SWITCH(cutlass::ceil_div(params.seqlen_q * (!PackGQA ? 1 : params.h / params.h_k), kBlockM) % 2 == 0, Use_cluster, [&] {
                                ...
                                // Disable PackGQA when using arbitrary mask
                                run_flash_fwd<Arch, kHeadDim, kHeadDimV, ClusterM, T, T_out, Is_causal, Is_local, Has_softcap, Varlen, PagedKVNonTMA, AppendKV && Varlen, HasQv, PackGQA && !Is_arbitrary, Split, V_colmajor, Is_arbitrary, kNFunc>(params, stream);
                            });
                        });
                    });
                });
