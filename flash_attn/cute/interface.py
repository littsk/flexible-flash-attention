...
def _flash_attn_fwd(
    ...
    if compile_key not in _flash_attn_fwd.compile_cache:
        logger.warning(
            "[FFA_FA4] JIT compiling fwd kernel: dtype=%s hdim=%s hdim_v=%s gqa=%s "
            "causal=%s arbitrary=%s nfunc=%s sparse=%s sm=%s (cache_size=%d)",
            dtype, head_dim, head_dim_v, qhead_per_kvhead, causal,
            arbitrary, func_num, use_block_sparsity, compute_capability,
            len(_flash_attn_fwd.compile_cache),
        )
...
def _flash_attn_bwd(
    ...
    if compile_key_pre not in _flash_attn_bwd.compile_cache_pre:
        logger.warning(
            "[FFA_FA4] JIT compiling bwd_pre kernel: sm=%s dtype=%s hdim_v=%s "
            "m_block=%s threads=%s (cache_size=%d)",
            compute_capability, dtype, head_dim_v, m_block_size, num_threads,
            len(_flash_attn_bwd.compile_cache_pre),
        )
        # Only create from_dlpack tensors when compilation is needed
    ...
    if compile_key not in _flash_attn_bwd.compile_cache:
        logger.warning(
            "[FFA_FA4] JIT compiling bwd kernel: dtype=%s hdim=%s hdim_v=%s gqa=%s "
            "causal=%s arbitrary=%s nfunc=%s sparse=%s deterministic=%s sm=%s "
            "(cache_size=%d)",
            dtype, head_dim, head_dim_v, qhead_per_kvhead, causal,
            arbitrary, func_num, use_block_sparsity, deterministic,
            compute_capability, len(_flash_attn_bwd.compile_cache),
        )
        # Only create from_dlpack tensors when compilation is needed
    ...
    if compile_key_post not in _flash_attn_bwd.compile_cache_post:
        logger.warning(
            "[FFA_FA4] JIT compiling bwd_post (dq) kernel: dtype=%s hdim=%s "
            "m_block=%s threads=%s (cache_size=%d)",
            dtype, head_dim, m_block_size, num_threads,
            len(_flash_attn_bwd.compile_cache_post),
        )
        # Only create from_dlpack tensors when compilation is needed
        dq_accum_tensor = from_dlpack(dq_accum.detach(), assumed_align=16, enable_tvm_ffi=True).mark_layout_dynamic(leading_dim=dq_accum.ndim - 1)
    ...
    if qhead_per_kvhead > 1:
        ...
        if compile_key_post not in _flash_attn_bwd.compile_cache_post:
            logger.warning(
                "[FFA_FA4] JIT compiling bwd_post (dk) kernel: dtype=%s hdim=%s "
                "n_block=%s threads=%s (cache_size=%d)",
        ...
        if compile_key_post not in _flash_attn_bwd.compile_cache_post:
            logger.warning(
                "[FFA_FA4] JIT compiling bwd_post (dv) kernel: dtype=%s hdim_v=%s "
                "n_block=%s threads=%s (cache_size=%d)",
...
def _flash_attn_fwd_combine(
    ...
    if compile_key not in _flash_attn_fwd_combine.compile_cache:
        logger.warning(
            "[FFA_FA4] JIT compiling fwd_combine kernel: hdim=%s m_block=%s "
            "k_block=%s log_max_splits=%s (cache_size=%d)",
            head_dim, m_block_size, k_block_size, log_max_splits,
            len(_flash_attn_fwd_combine.compile_cache),
        )
        # Only create from_dlpack tensors when compilation is needed
