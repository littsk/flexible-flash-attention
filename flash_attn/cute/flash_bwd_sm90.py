...
from flash_attn.cute.cp_sync_sm90 import wait_kv_ready, fence_proxy_async_global
...
class FlashAttentionBackwardSm90:
    ...
    def __call__(
        ...
        assert getattr(self, "dkv_done_mc_ptr", None) is None, "SM90 CP uses local release counters"
        ...
        bwd_kv_order = blocksparse_tensors.bwd_kv_order if const_expr(blocksparse_tensors is not None) else None
        bwd_work_map = blocksparse_tensors.bwd_work_map if const_expr(blocksparse_tensors is not None) else None
        if const_expr(bwd_kv_order is not None):
            TileScheduler = ThreePhaseBwdSingleTileScheduler
        elif const_expr(mCuSeqlensK is not None or mSeqUsedK is not None):
            TileScheduler = SingleTileVarlenScheduler
        elif const_expr(self.deterministic):
            TileScheduler = SingleTileLPTBwdScheduler
        else:
            TileScheduler = SingleTileScheduler
        self.spt = getattr(self, "spt_override", (self.is_causal or self.is_local)) and self.deterministic
        ...
        if warp_idx_in_wg == 0:
            ...
            while work_tile.is_valid_tile:
                ...
                if const_expr(blocksparse_tensors is not None):
                    active_q = get_total_q_block_count_bwd(
                        blocksparse_tensors, batch_idx, head_idx, n_block,
                        subtile_factor=self.subtile_factor,
                        m_block_max=block_info.get_m_block_min_max(seqlen, n_block)[1],
                    )
                    if active_q > 0:
                        wait_kv_ready(blocksparse_tensors.kv_block_signal, head_idx_kv, n_block)
    ...
    def mma(
        ...
        while work_tile.is_valid_tile:
            ...
            if const_expr(mdKV_done is not None or mdGQA_local_done is not None):
                if process_tile:
                    ...
                    if tidx == 0:
                        ...
                        # Full async completion is ordered before a system release.
                        if const_expr(mdGQA_local_done is not None):
                            flat = (head_idx // qhead_per_kvhead_divmod) * self.gqa_nblk + n_block
                            if const_expr(mdGQA_local_expected is not None and mdGQA_finalize_state is not None):
                                previous = cute.arch.atomic_add(mdGQA_local_done.iterator + flat, Int32(1), sem="acq_rel", scope="sys")
                                if previous + Int32(1) == mdGQA_local_expected[flat]:
                                    cute.arch.atomic_add(mdGQA_finalize_state.iterator + Int32(4) + flat, Int32(1), sem="release", scope="sys")
                                    cute.arch.atomic_add(mdGQA_finalize_state.iterator, Int32(1), sem="release", scope="sys")
                            else:
                                cute.arch.atomic_add(mdGQA_local_done.iterator + flat, Int32(1), sem="release", scope="sys")
                        else:
                            flat = head_idx * self.dkv_done_nblk + n_block
                            cute.arch.atomic_add(mdKV_done.iterator + flat, Int32(1), sem="release", scope="sys")
    ...
    def dQaccum_store(
        ...
        while work_tile.is_valid_tile:
            ...
            if process_tile:
                if const_expr(not self.use_block_sparsity):
                    ...
                    dQaccum_store_block_sparse_bwd_sm90(
                        ...
                        semaphore=mdQ_semaphore_cur if const_expr(self.deterministic) else None,
                    )
            ...
            if const_expr(
                self.deterministic and not self.spt and not self.use_block_sparsity
