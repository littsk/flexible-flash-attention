...
class FlashAttentionForwardSm90(FlashAttentionForwardBase):
    ...
    def _get_shared_storage_cls(self):
        ...
        sQ_struct = cute.struct.Align[cute.struct.MemRange[self.dtype, max(cute.cosize(self.sQ_layout), output_elems)], self.buffer_align_bytes]
    ...
    def __call__(
        ...
        O_layout_transpose = [2, 4, 3, 1, 0] if const_expr(self.is_split_kv) else QO_layout_transpose
        ...
        if const_expr(self.is_split_kv and blocksparse_tensors.local_mask_block_cnt is not None):
        ...
        tile_sched_args = TileSchedulerArguments(
            ...
            fwd_work_order=blocksparse_tensors.fwd_work_order if const_expr(blocksparse_tensors is not None) else None,
    ...
    def kernel(
        ...
        sO = storage.sQ.get_tensor(sO_layout.outer, swizzle=sO_layout.inner, dtype=self.output_dtype)
        ...
        if is_load_warp:
            ...
            while work_tile.is_valid_tile:
                ...
                if const_expr(self.use_tma_KV):
                    ...
                    if const_expr(blocksparse_tensors is not None and blocksparse_tensors.kv_block_signal is not None):
                        tma_load_K_fn = partial(
                            gated_k_load, tma_load_K_fn,
                            blocksparse_tensors.kv_block_signal, head_idx_kv,
                        )
