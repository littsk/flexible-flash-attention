...
class PipelineTmaUmma(PipelineTmaUmmaOg):
    ...
    NOTE: We deliberately do NOT reimplement ``create()`` here. Instead we
    delegate to ``PipelineTmaUmmaOg.create()`` (cutlass.pipeline.PipelineTmaUmma)
    and rebind ``__class__`` so the returned instance picks up our overrides.
    This mirrors upstream Dao-AILab/flash-attention commit ``deb1830`` and
    is required for nvidia-cutlass-dsl >= 4.4.2 compatibility: in 4.4.2,
    ``PipelineAsync._make_sync_object`` (sm90) no longer accepts
    ``PipelineOp.TCGen05Mma`` (it now lives only on the sm100
    ``PipelineTmaUmma._make_sync_object`` override), so any hand-rolled
    ``create()`` that called ``PipelineAsync._make_sync_object`` would assert
    "Invalid PipelineOp specified.".
    """
